...
public class MapsActivity extends FragmentActivity implements GoogleMap.OnMapLongClickListener {
    ...
    private void setUpMap() {
        if (model.getModeNewPosition() == 2) {
            ...
            mMap.addMarker(new MarkerOptions().position(new LatLng(model.getPosLatitude(), model.getPosLongitude())).title("You're here"));
        }
    }
    ...
    public void onMapLongClick(LatLng latLng) {
        ...
        model.setPosLatitude(latLng.latitude);
        model.setPosLongitude(latLng.longitude);
        this.finish();
    }
}
